class Schema::Parser
  ...
  def parse_whole_value(raw_value)
    return Schema::Node::Nothing.new if raw_value.empty?
  ...
  def parse_value(raw_value)
    ...
    if PRIMITIVE_KINDS.include?(value)
      Schema::Node::Primitive.new(kind: value)
    elsif value[0] == "{"
      parse_object(value)
    elsif value[0] == "["
      parse_array(value)
  ...
  def parse_one_of(str)
    Schema::Node::OneOf.new(branches: split_by_pipe(str[1...-1]).map { |branch| parse_value(branch) })
  end
  ...
  def parse_entity(value)
    ...
    Schema::Node::Entity.new(entity: valid_entity)
  end
