...
describe EntityReferences do
  ...
  describe "#names_reaching" do
    it "names only the entity itself when nothing points at it" do
      ...
      expect(EntityReferences.new(entities).names_reaching("Customer")).to eq(%w[Customer])
    end

    it "names whoever points at it" do
      entities = entities_with("Customer" => "{id:string}", "Order" => "{customer:Customer}")

      expect(EntityReferences.new(entities).names_reaching("Customer")).to match_array(%w[Customer Order])
    end
    ...
    it "follows the chain all the way back" do
      ...
      expect(EntityReferences.new(entities).names_reaching("Address")).to match_array(%w[Address Customer Order])
      expect(EntityReferences.new(entities).names_reaching("Order")).to eq(%w[Order])
    end
    ...
    it "looks through arrays and unions" do
      ...
      expect(EntityReferences.new(entities).names_reaching("Customer")).to match_array(%w[Customer LineItem Order])
    end
  end
