...
describe Schema::ExpandedLineIndex, type: :model do
  ...
  it "consumes a single row for an entity whose root is a primitive" do
    ...
    value = Schema::Parser.new([ tag_entity ]).parse_value("{tag:Tag,count:number}")
  ...
  it "counts a reference nested inside a reference, since expansion goes all the way down" do
    ...
    value = Schema::Parser.new(entities).parse_value("{owner:Customer,count:number}")
  ...
  it "uses the current size of an entity whose root gained a field" do
    ...
    value = Schema::Parser.new([ new_user ]).parse_value("{total:number,items:[User]}")
