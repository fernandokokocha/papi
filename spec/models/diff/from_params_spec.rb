...
describe Diff::FromParams do
  ...
  describe "#any_changes?" do
    it "is false when both sides hold the same kinds" do
      diff = Diff::FromParams.new([ param("id", "number") ], [ param("id", "number") ])
      expect(diff.any_changes?).to be(false)
    end

    it "is true when a kind changes" do
      diff = Diff::FromParams.new([ param("id", "string") ], [ param("id", "number") ])
      expect(diff.any_changes?).to be(true)
    end

    it "is false when neither side has params" do
      expect(Diff::FromParams.new([], []).any_changes?).to be(false)
    end
  end
  ...
  describe "query params" do
    ...
    it "renders a required param bare and an optional one with a question mark" do
      diff = Diff::FromParams.new([], [ query("q", "string", required: true), query("page", "number") ])
    ...
    it "marks an optional param becoming required as type_changed" do
      diff = Diff::FromParams.new([ query("page", "number") ], [ query("page", "number", required: true) ])
    ...
    it "is unchanged when kind and requiredness both hold" do
      diff = Diff::FromParams.new([ query("page", "number") ], [ query("page", "number") ])
  ...
  describe "Row#removed?" do
    it "is true only for a param the new side dropped" do
      diff = Diff::FromParams.new([ param("id", "number"), param("slug", "string") ], [ param("id", "number") ])
