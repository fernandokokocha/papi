...
describe Diff::FromAuth do
  ...
  describe "#any_changes?" do
    it "is false when both sides name the same method" do
      diff = Diff::FromAuth.new(auth("UserToken", "bearer"), auth("UserToken", "bearer"))
      expect(diff.any_changes?).to be(false)
    end

    it "is false when neither side declares auth" do
      expect(Diff::FromAuth.new(nil, nil).any_changes?).to be(false)
    end

    it "is true when an endpoint switches to another method" do
      diff = Diff::FromAuth.new(auth("UserToken", "bearer"), auth("AdminBasic", "basic"))
    ...
    it "is true when the method keeps its name but changes kind" do
      diff = Diff::FromAuth.new(auth("UserToken", "bearer"), auth("UserToken", "basic"))
      expect(diff.any_changes?).to be(true)
    end
  end
  ...
  describe "the rendered lines" do
    it "marks an endpoint that gained auth as added, leaving the absent side alone" do
      diff = Diff::FromAuth.new(nil, auth("UserToken", "bearer"))
    ...
    it "marks an endpoint that lost auth as removed, leaving the absent side alone" do
      diff = Diff::FromAuth.new(auth("UserToken", "bearer"), nil)
