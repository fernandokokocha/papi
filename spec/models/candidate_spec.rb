...
describe Candidate do
  describe "#comment_threads_by_anchor" do
    ...
    it "groups root comments by anchor key and excludes replies" do
      ...
      expect(map[[ "endpoint", "/users", 0, nil, nil, nil, nil, nil, "whole", nil ]]).to eq([ root ])
      expect(map[[ "candidate", nil, nil, nil, nil, nil, nil, nil, "whole", nil ]]).to eq([ candidate_level ])
    ...
    it "sorts threads within an anchor by creation time" do
      ...
      expect(map[[ "candidate", nil, nil, nil, nil, nil, nil, nil, "whole", nil ]]).to eq([ older, newer ])
    end
  end
